...
- (void)share_ext_set_intent_action:(int)intentAction {
    //NSLog(@"share_ext_set_intent_action");
}
...
- (int)share_ext_launch {
    ...
    if ([activityType isEqualToString:UIActivityTypePostToFacebook] ||
        ...
        [composeViewController setCompletionHandler:^(SLComposeViewControllerResult result) {
            ...
            [gmshare_chosenActivityTypeString release];
            gmshare_chosenActivityTypeString = [activityType retain];
        }];
        ...
        composeViewController.recipients = gmshare_recipients;
        ...
        [composeViewController setToRecipients:gmshare_recipients];
...
- (void)showActivityViewControllerWithItems:(NSMutableArray *)sharedItems useExt:(BOOL)useExt {
    ...
    NSMutableArray *finalSharedItems = [NSMutableArray new];
    for (id obj in sharedItems) {
        if ([obj isKindOfClass:[NSDictionary class]]) {
            [finalSharedItems addObject:obj[@"image"]];
        }
        else {
            [finalSharedItems addObject:obj];
        }
    }
    gmshare_didFinish = NO;
    gmshare_completed = NO;
    gmshare_chosenActivityType = -1;
    gmshare_chosenActivityTypeString = @"";
    UIActivityViewController *activityViewController = [[UIActivityViewController alloc] initWithActivityItems:finalSharedItems applicationActivities:nil];
    [finalSharedItems release];
    NSString *useSubject = @"";
    ...
    if (useExt && [gmshare_excludedActivityTypes count] > 0) {
        activityViewController.excludedActivityTypes = gmshare_excludedActivityTypes;
    }
